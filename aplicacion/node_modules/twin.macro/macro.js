...
var isCssClass = function (className) { return className.includes('['); };
...
var getProperties = function (className, state, ref) {
  ...
  if (!className) { return; }
  var isCss = isCssClass(className);
  if (isCsOnly || className.includes('[')) { return {
    hasMatches: isCss,
    type: 'css'
  }; }
  ...
  var type = isCss && 'css' || isStatic && 'static' || isDynamicClass && 'dynamic' || corePlugin && 'corePlugin';
  return {
    type: type,
...
var splitNegative = function (ref) {
  ...
  var isShortCss = className.includes('[');
  var hasNegative = !isShortCss && className.slice(0, 1) === '-'; // TODO: Look in deprecating the negative prefix removal
